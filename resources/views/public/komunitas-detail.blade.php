...
<head>
    ...
    <title>{{ $komunitas->name }} - Laboratorium Sistem Informasi</title>
    ...
    <style>
        ...
        .prose p { color: #374151; margin-top: 1em; margin-bottom: 1em; }
            ...
            <div class="container mx-auto px-4 md:px-6">
                <div class="text-center mb-12">
                    ...
                    @if($komunitas->logo)
                        ...
                        <img src="{{ asset('storage/' . $komunitas->logo) }}" alt="Logo {{ $komunitas->name }}" class="mx-auto mb-6 h-32 w-32 object-cover rounded-2xl shadow-lg border-4 border-white">
                    @endif

                    {{-- Nama Komunitas --}}
                    {{-- PERUBAHAN: Margin atas (mt) dihilangkan agar lebih dekat dengan logo --}}
                    <h1 class="text-4xl md:text-6xl font-extrabold tracking-tight text-blue-600">{{ $komunitas->name }}</h1>
                    
                    {{-- Tagline Komunitas --}}
                    @if($komunitas->tagline)
                        <p class="text-lg md:text-xl mt-2 text-gray-600 max-w-2xl mx-auto">{{ $komunitas->tagline }}</p>
                    @endif
                </div>
                    ...
                    <div class="bg-white p-4 rounded-2xl shadow-lg">
                        <div class="p-6 border-b">
                            <h2 class="text-xl font-semibold flex items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6 text-blue-600"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        ...
                        <div class="p-6 space-y-8">
                            <div>
                                ...
                                <div class="flex-1 min-w-[150px] text-center border rounded-lg p-4 hover:shadow-lg transition-shadow">
                                    {{-- Cek apakah pimpinan punya foto. Jika tidak, tampilkan inisial. --}}
                                    @if($pimpinan->photo)
                                        {{-- Jika ada foto, tampilkan foto tersebut --}}
                                        <img class="w-24 h-24 mb-3 rounded-full shadow-lg mx-auto object-cover" src="{{ $pimpinan->photo ? asset('storage/' . $pimpinan->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($pimpinan->name) . '&background=0D8ABC&color=fff' }}" alt="{{ $pimpinan->name }}"
                                    @else
                                        {{-- Jika tidak ada foto, tampilkan lingkaran dengan inisial nama --}}
                                        @php
                                        ...
                                        @endphp
                                        <div class="w-20 h-20 mb-2 border-2 border-blue-600 rounded-full mx-auto flex items-center justify-center bg-gray-200 text-2xl font-semibold text-gray-600">
                                            {{ $initials }}
                                        </div>
                                    @endif
                            ...
                            <div>
                                ...
                                <span class="bg-gray-100 text-gray-800 text-sm font-medium px-3 py-1.5 rounded-full">{{ $anggota->name }}</span>
                                @endforeach
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="bg-white p-8 rounded-2xl shadow-lg">
                             <h2 class="text-3xl font-bold text-gray-900 mb-6 flex items-center">
                                <svg class="w-8 h-8 mr-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                Agenda Kegiatan
                            </h2>
                            <div class="space-y-6">
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <p class="text-gray-500 text-sm font-medium">{{ $agenda->schedule->translatedFormat('l, d F Y - H:i') }} WIB</p>
